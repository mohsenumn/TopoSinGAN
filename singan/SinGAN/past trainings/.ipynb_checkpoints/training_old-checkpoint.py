...
# def hanging_line_loss(edge_mask):
#     # Define a 3x3 kernel to count the number of active neighbors
#     kernel = torch.ones((1, 1, 3, 3), dtype=torch.float32)
#     kernel[0, 0, 1, 1] = 0  # we exclude the center pixel
    
#     # Convolve the binary edge mask with the kernel
#     neighbor_count = F.conv2d(edge_mask, kernel, padding=1)
    
#     # Soft approximation for leaf_nodes
#     leaf_approx = torch.sigmoid(10 * (1 - neighbor_count)) * edge_mask
    
#     # Soft approximation for junction_nodes
#     junction_approx = torch.sigmoid(10 * (neighbor_count - 2.5)) * edge_mask
    
#     # Count approximated leaf and junction nodes
#     num_leaf_approx = leaf_approx.sum()
#     num_junction_approx = junction_approx.sum()
    
#     # Calculate the loss as ratio of leaf nodes to junction nodes
#     # To ensure we don't divide by zero, we add a small epsilon
#     loss = num_leaf_approx / (num_junction_approx + 1e-8)
#     return num_leaf_approx

# def custom_topo_loss(fake, real):
#     return 1-hanging_line_loss(real)/hanging_line_loss(fake)


def softplus_inv(x):
    return torch.log(torch.exp(x) - 1)

def hanging_line_loss(input_tensor):
    # Extracting the fourth channel for the mask
    edge_mask = input_tensor[:, 3:4, :, :]
    ...
    # Calculate the loss as the ratio of leaf nodes to junction nodes
    # To ensure we don't divide by zero, we add a small epsilon
    loss = num_leaf_approx / (num_junction_approx + 1e-8)
    return num_leaf_approx

def custom_topo_loss(fake, real):
    return 1 - hanging_line_loss(real) / (hanging_line_loss(fake)+ 1e-8)
...
def train(opt,Gs,Zs,reals,NoiseAmp, seed_for_img_dir):
    ...
    while scale_num<opt.stop_scale+1:
        ...
        z_curr,in_s,G_curr = train_single_scale(D_curr,G_curr,reals,Gs,Zs,in_s,NoiseAmp,opt, scale_num)
...
def train_single_scale(netD,netG,reals,Gs,Zs,in_s,NoiseAmp,opt,scale_num, centers=None):
...
#                     plt.imsave('%s/noise_amp_p_z_opt.png' % (opt.outf), functions.convert_image_np(opt.noise_amp*z_opt.float()), vmin=0, vmax=1)
                ...
                ## TOPO LOSS
                loss2 = custom_topo_loss(netG(Z_opt.detach(),z_prev),real)
#                 top_loss = alpha*topo_loss
                loss2.backward(retain_graph=True)
                top_loss = loss2.detach()
        ...
        if epoch % 25 == 0 or epoch == (opt.niter-1):
            ...
            print('top_loss = ', top_loss, 'rec_loss = ', rec_loss, 'errG= ', errG)
