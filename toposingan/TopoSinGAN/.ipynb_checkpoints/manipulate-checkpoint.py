...
import TopoSinGAN.functions
import TopoSinGAN.models
import argparse
import os
import random
...
#                     dir2save = functions.generate_dir2save(opt)
                ...
                if (opt.mode != "harmonization") & (opt.mode != "editing") & (opt.mode != "SR") & (opt.mode != "paint2image"):
                    ...
                    plt.imsave('%s/%d_m.png' % (dir2save, i), functions.convert_image_np(I_curr.detach())[:,:,3], vmin=0,vmax=1) # Vajira
                    # plt.imsave('%s/%d_rgbmask.png' % (dir2save, i), functions.convert_image_np(I_curr.detach()), vmin=0,vmax=1) 
                    plt.imsave('%s/%d_rgb.png' % (dir2save, i), functions.convert_image_np(I_curr.detach())[:,:,0:3], vmin=0,vmax=1) # Vajira
