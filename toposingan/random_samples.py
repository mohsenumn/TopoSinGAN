...
from TopoSinGAN.imresize import imresize
import TopoSinGAN.functions as functions


if __name__ == '__main__':
    parser = get_arguments(seed_for_img_dir = 0)
    ...
    parser.add_argument('--num_samples', type=int, help="number of sampels to generate random samples", default=5)
    parser.add_argument('--gpu_id', help='GPU ID to train')
    opt = parser.parse_args()
    opt = functions.post_config(opt, seed_for_img_dir=0)
    Gs = []
    Zs = []
    reals = []
    NoiseAmp = []
#     dir2save = functions.generate_dir2save(opt, seed_for_img_dir=0)
    dir2save = '/SAMPLES_Bostontopo'
    if dir2save is None:
        ...
        if opt.mode == 'random_samples':
            ...
            basep = 'TrainedModels_554/boston/scale_factor=0.75,alpha=10/'
            Gs = torch.load(basep + 'Gs.pth')
            Zs = torch.load(basep + 'Zs.pth')
            reals = torch.load(basep + 'reals.pth')
            NoiseAmp = torch.load(basep + 'NoiseAmp.pth')
            
#             Gs, Zs, reals, NoiseAmp = functions.load_trained_pyramid(opt)
            in_s = functions.generate_in2coarsest(reals,1,1,opt)
            SinGAN_generate(Gs, Zs, reals, NoiseAmp, opt, gen_start_scale=opt.gen_start_scale, num_samples=50)
